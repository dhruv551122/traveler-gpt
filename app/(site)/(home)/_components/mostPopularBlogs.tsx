            ...
            <div className="grid lg:grid-cols-2 gap-4 lg:gap-8">
                {
                    popularBlogs.map((blog, index) => <Link href={`/blogs/${blog.slug.current}`} key={blog._id} className="flex w-fit group gap-4 ">
                        <span className="p-1 shrink-0 border size-10 flex items-center justify-center text-center font-signika-negative rounded-full group-hover:text-white group-hover:bg-deep-bright-red/80 duration-300 group-hover:border-deep-bright-red border-gunmetal-black subtitle-2">{index + 1}</span>
